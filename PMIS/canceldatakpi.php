...
<table  width="100%" >
		  ...
		  <div id="activities"  <?php echo $style; ?>> 
			...
			$sqlg="Select * from maindata where stage='Activity' and isentry=1 and itemid not in(SELECT DISTINCT(b.itemid) as actid1 FROM kpi_activity a inner join activity b on (a.activityid=b.aid) where a.kpiid=$itemid)";
			...
			?>
			  ...
			  ?>
			   ...
			   <?php  if($kpientry_flag==1 || $kpiadm_flag==1)
				{
				?>
			    <input type="button" value="Add Activities" name="save" id="save" onclick="addactivities(<?php echo $itemid; ?>)" /><br />
<?php
}
?>
...
<?php 
$sql_b="SELECT a.kaid, b.aid,b.itemid,b.code,b.secheduleid,b.startdate,b.enddate,b.actualstartdate,b.actualenddate, b.aorder, b.baseline, b.remarks, b.rid FROM `kpi_activity` a inner join activity b on (a.activityid=b.aid) where a.kpiid=".$itemid; 
