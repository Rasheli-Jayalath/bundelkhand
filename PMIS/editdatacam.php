...
$sql_a="SELECT b.aid,b.itemid,b.code,b.secheduleid,b.startdate,b.enddate,b.actualstartdate,b.actualenddate, b.aorder, b.baseline, b.remarks, b.rid FROM `cam_activity` a inner join activity b on (a.activityid=b.aid) where a.caid=".$itemid; 
			...
			<?php
			$sqlga="Select * from cam_activity where activityid=$row3[aid] and caid=$itemid";
			$resga=mysql_query($sqlga);
			$rowa_a=mysql_fetch_array($resga);
			$camid=$rowa_a['camid'];
			?>
			
			<td><input id="caweight" name="caweight" type="text" value="<?=$rowa_a['caweight'];?>"/></td>
			<td>
			<?php
				  if($camentry_flag==1 || $camadm_flag==1)
				{
				?>
			<input type="button" value="Update" name="update" id="update"  onclick="update_data(<?php echo $camid;?>)"/>
			<?php
			}
			?></td>
			</tr>
			...
			$sqlga1="Select * from cam_activity where activityid=$row3[aid] and caid=$itemid";
			$resga1=mysql_query($sqlga1);
			$rowa_a1=mysql_fetch_array($resga1);
			?>
			
			<td><?=$rowa_a1['caweight'];?></td>
			<td><?php
				  if($camentry_flag==1 || $camadm_flag==1)
				{
