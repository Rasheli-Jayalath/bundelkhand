...
$module="CAM Activity Data Entry";
if ($uname==null)
{
		...
		$eSql_l = "Select * from activity where itemid=".$arr_act[$i];
	 	...
	 	$numrows=mysql_num_rows($res_sql);
			if($numrows>0)
			{
				...
				{
				$aid=$rows['aid'];
					$eSql_2 = "Select * from cam_activity where caid=$itemid and activityid=$aid";
					...
					{
					$sSQL = ("INSERT INTO cam_activity (caid,activityid) VALUES ($itemid,$aid)");
					$objDb->execute($sSQL);
					}
...
<table  width="100%">
		  ...
		  <div id="activities"  <?php echo $style; ?>> 
			...
			while($row3g=mysql_fetch_array($resg))
			{
			if($row3g['schedulecode']==$row3_a['secheduleid'])
				{
						
			?>
			
			<td><?=$row3_a['secheduleid'].": ".$row3g['resource'];?></td>
			<?php
			}
			}
			?>
			...
			<?php
			$sqlga="Select * from cam_activity where activityid=$aid and caid=$itemid";
