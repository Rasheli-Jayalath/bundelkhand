...
$latest_achieved=0;
$reportquery ="SELECT z.itemid,z.itemname, z.itemcode, z.parentcd,z.activitylevel,z.weight, min(b.startdate) as startdate, max(b.enddate) as enddate, sum(b.baseline) as baseline,z.parentgroup, z.activitylevel FROM maindata z inner join milestone_activity a on (z.itemid=a.milestoneid) inner join activity b on (a.activityid=b.aid) inner join mildata c on (b.itemid=c.itemid AND b.rid=c.rid) where z.parentcd=".$data_level_id."  Group by z.itemid";
//$reportquery ="SELECT aid, baseline as milestoneBaseline, assig FROM mis_tbl_4_milstones where s_id=".$s_id." Group by s_id,aid order by s_id";
$reportresult = mysql_query($reportquery);
$counter=mysql_num_rows($reportresult);
while ($reportdata = mysql_fetch_array($reportresult)) {
...
function getMilestoneTargetsCCC($lastMonth,$itemid)
{
	...
	$milestone_targets="TGC".$month;
	 $sql="SELECT  sum(`".$milestone_targets."`) as milestone_targets FROM maindata z inner join milestone_activity a on (z.itemid=a.milestoneid) inner join 
	 activity b 
	 on (a.activityid=b.aid) inner join mildata c on (b.itemid=c.itemid AND b.rid=c.rid) where a.milestoneid=".$itemid." Group by z.itemid";
 $amountresult = mysql_query($sql);
if($amountresult!=0)
{
 $data=mysql_fetch_array($amountresult);
...
function getMilestoneTotalAchieveCLatest($lastMonth,$itemid)
{
 ...
 $sql="SELECT  sum(`".$milestone_progress."`) as milestone_achieved FROM maindata z inner join milestone_activity a on (z.itemid=a.milestoneid) inner join 
	 activity b 
	 on (a.activityid=b.aid) inner join mildata c on (b.itemid=c.itemid AND b.rid=c.rid) where a.milestoneid=".$itemid." Group by z.itemid";
...
function getMilestoneTotalAchieveCLast($lastMonth,$itemid)
{
	$lastMonth=date('Y-m-d',strtotime($lastMonth));
	 $m=date('m',strtotime($lastMonth));
	 $y=date('Y',strtotime($lastMonth));
...
function getMilestoneTotalTargetsCLast($lastMonth,$itemid)
{
		$lastMonth=date('Y-m-d',strtotime($lastMonth));
	 $m=date('m',strtotime($lastMonth));
	 $y=date('Y',strtotime($lastMonth));
     ...
     $month = str_replace("-","",$lastMonth); // Date var
	$milestone_targets="TGC".$month;
	$sql="SELECT  sum(".$milestone_targets.") as milestone_targets FROM maindata z inner milestone_activity a on (z.itemid=a.milestoneid) inner join 
	 activity b 
