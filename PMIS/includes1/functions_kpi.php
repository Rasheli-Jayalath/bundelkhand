...
function getMilestoneTargetsCCC($month,$itemid)
{
	 $lastMonth=date('Y-m-d',strtotime($month));
	...
	$milestone_targets="TGC".$month;
	 $sql="SELECT  sum(".$milestone_targets.") as milestone_targets FROM maindata z inner join kpi_activity a on (z.itemid=a.kpiid) inner join 
	 activity b 
...
function getMilestoneTotalTargetsCLatest($latest_month,$itemid)
{
$lastMonth=date('Y-m-d',strtotime($latest_month));
	 $m=date('m',strtotime($lastMonth));
	 $y=date('Y',strtotime($lastMonth));
	 $days=cal_days_in_month(CAL_GREGORIAN, $m, $y); 
	 $lastMonth=$y."-".$m."-".$days;
...
$month = str_replace("-","",$lastMonth); // Date var
	$milestone_progress="ACC".$month;
	 $sql="SELECT  sum(".$milestone_progress.") as milestone_achieved FROM maindata z inner join kpi_activity a on (z.itemid=a.kpiid) inner join 
	 activity b 
	 on (a.activityid=b.aid) inner join mildata c on (b.itemid=c.itemid AND b.rid=c.rid) where kpiid=".$itemid." Group by z.itemid";
...
{
     $lastMonth=date('Y-m-d',strtotime($last_month));
	 $m=date('m',strtotime($lastMonth));
	 $y=date('Y',strtotime($lastMonth));
	 $days=cal_days_in_month(CAL_GREGORIAN, $m, $y); 
	 $lastMonth=$y."-".$m."-".$days;
