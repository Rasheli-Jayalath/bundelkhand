...
function getProjectCommProgC($lastMonth,$data_level_id,$latest_month)
{
...
while ($reportdata = mysql_fetch_array($reportresult)) {
   ...
   if($latest_month!=0)
	  ...
	  $latest_achieved=getComponentCommProgC($lastMonth,$reportdata['itemid'],$latest_month);
...
$latest_achieved=0;
$reportquery ="SELECT z.itemid,z.itemname, z.itemcode, z.parentcd,z.activitylevel,z.weight, min(b.startdate) as startdate, max(b.enddate) as enddate, sum(b.baseline) as baseline,z.parentgroup,z.activitylevel FROM maindata z left outer join cam_activity a on (z.itemid=a.caid) left outer join activity b on (a.activityid=b.aid) left outer join mildata c on (b.itemid=c.itemid AND b.rid=c.rid) where z.parentcd=".$data_level_id." Group by z.itemid";
//echo $reportquery ="SELECT z.itemid,z.itemname, z.itemcode, z.parentcd,z.activitylevel,z.weight, min(b.startdate) as startdate, max(b.enddate) as enddate, sum(b.baseline) as baseline,z.parentgroup, z.activitylevel FROM maindata z inner join cam_activity a on (z.itemid=a.caid) inner join activity b on (a.activityid=b.aid) inner join mildata c on (b.itemid=c.itemid AND b.rid=c.rid) where z.parentcd=".$data_level_id."  Group by z.itemid";
$reportresult = mysql_query($reportquery);
$counter=mysql_num_rows($reportresult);
while ($reportdata = mysql_fetch_array($reportresult)) {
	 //$latest_month=getLatestMonth($reportdata['aid']);
...
$latest_achieved=0;
$reportquery ="SELECT z.itemid,z.itemname, z.itemcode, z.parentcd,z.activitylevel,z.weight, min(b.startdate) as startdate, max(b.enddate) as enddate, sum(b.baseline) as baseline,z.parentgroup, z.activitylevel FROM maindata z inner join cam_activity a on (z.itemid=a.caid) inner join activity b on (a.activityid=b.aid) inner join mildata c on (b.itemid=c.itemid AND b.rid=c.rid) where z.parentcd=".$data_level_id."  Group by z.itemid";
//$reportquery ="SELECT aid, baseline as milestoneBaseline, assig FROM mis_tbl_4_milstones where s_id=".$s_id." Group by s_id,aid order by s_id";
$reportresult = mysql_query($reportquery);
$counter=mysql_num_rows($reportresult);
while ($reportdata = mysql_fetch_array($reportresult)) {
...
function getProjectCommTargC($lastMonth,$data_level_id,$latest_month)
{
...
while ($reportdata = mysql_fetch_array($reportresult)) {
	...
	if($latest_month!=0)
	  ...
	  $latest_target=getComponentCommTargC($lastMonth,$reportdata['itemid'],$latest_month);
...
$latest_target=0;
/*echo $reportquery ="SELECT z.itemid,z.itemname, z.itemcode, z.parentcd,z.activitylevel,z.weight, min(b.startdate) as startdate, max(b.enddate) as enddate, sum(b.baseline) as baseline,z.parentgroup,z.activitylevel FROM maindata z left outer join cam_activity a on (z.itemid=a.caid) left outer join activity b on (a.activityid=b.aid) left outer join mildata c on (b.itemid=c.itemid AND b.rid=c.rid) where z.parentcd=".$data_level_id." Group by z.itemid";*/
...
function getMilestoneTargetsCCC($lastMonth,$itemid)
{
	$lastMonth=date('Y-m-d',strtotime($lastMonth));
     ...
     $month = str_replace("-","",$lastMonth); // Date var
	$milestone_targets="TGC".$month;
	$sql="SELECT  sum(`".$milestone_targets."`) as milestone_targets FROM maindata z inner join cam_activity a on (z.itemid=a.caid) inner join 
	 activity b 
	 on (a.activityid=b.aid) inner join mildata c on (b.itemid=c.itemid AND b.rid=c.rid) where a.caid=".$itemid." Group by z.itemid";
 $amountresult = mysql_query($sql);
if($amountresult!=0)
{
 $data=mysql_fetch_array($amountresult);
...
function getMilestoneTargetsPC($month,$itemid)
{
	$month = str_replace("-","",$month); // Date var
	$milestone_targets="TG".$month;
	 $sql="SELECT  sum(`".$milestone_targets."`) as milestone_targets FROM maindata z inner join cam_activity a on (z.itemid=a.caid) inner join 
	 activity b 
	 on (a.activityid=b.aid) inner join mildata c on (b.itemid=c.itemid AND b.rid=c.rid) where a.caid=".$itemid." Group by z.itemid";
	//echo "<br/>";
...
function getMilestoneTotalTargetsCLatest($lastMonth,$itemid)
{
$lastMonth=date('Y-m-d',strtotime($lastMonth));
	 $m=date('m',strtotime($lastMonth));
	 $y=date('Y',strtotime($lastMonth));
    ...
    $sql="SELECT  sum(`".$milestone_targets."`) as milestone_targets FROM maindata z inner join cam_activity a on (z.itemid=a.caid) inner join 
	 activity b 
	 on (a.activityid=b.aid) inner join mildata c on (b.itemid=c.itemid AND b.rid=c.rid) where caid=".$itemid." Group by z.itemid";
 $amountresult = mysql_query($sql);
...
function getMilestoneTotalAchieveCLatest($lastMonth,$itemid)
{
	$lastMonth=date('Y-m-d',strtotime($lastMonth));
	 $m=date('m',strtotime($lastMonth));
	 $y=date('Y',strtotime($lastMonth));
...
$lastMonth = str_replace("-","",$lastMonth);
$month = str_replace("-","",$lastMonth); // Date var
	$milestone_progress="ACC".$month;
 $sql="SELECT  sum(`".$milestone_progress."`) as milestone_achieved FROM maindata z inner join cam_activity a on (z.itemid=a.caid) inner join 
	 activity b 
	 on (a.activityid=b.aid) inner join mildata c on (b.itemid=c.itemid AND b.rid=c.rid) where a.caid=".$itemid." Group by z.itemid";
...
function getMilestoneTotalAchieveCLast($last_month,$itemid)
{
$month = str_replace("-","",$last_month); // Date var
	$milestone_progress="ACC".$month;
	 $sql="SELECT  sum(".$milestone_progress.") as milestone_achieved FROM maindata z inner join cam_activity a on (z.itemid=a.caid) inner join 
	 activity b 
	 on (a.activityid=b.aid) inner join mildata c on (b.itemid=c.itemid AND b.rid=c.rid) where caid=".$itemid." Group by z.itemid";
...
function getMilestoneTotalTargetsCLast($last_month,$itemid)
{
	$month = str_replace("-","",$last_month); // Date var
	$milestone_targets="TGC".$month;
	$sql="SELECT  sum(".$milestone_targets.") as milestone_targets FROM maindata z inner cam_activity a on (z.itemid=a.caid) inner join 
	 activity b 
	 on (a.activityid=b.aid) inner join mildata c on (b.itemid=c.itemid AND b.rid=c.rid) where kpiid=".$itemid." Group by z.itemid";
 $amountresult = mysql_query($sql);
if($amountresult!=0)
{
 $data=mysql_fetch_array($amountresult);
...
function getMilestoneAchieveCCC($month,$itemid)
{
...
function getMilestoneAchievePC($month,$itemid)
{

	$month = str_replace("-","",$month); // Date var
	$milestone_progress="AC".$month;
