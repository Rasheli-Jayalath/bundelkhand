...
function mildatamaker($itemid, $rid,$kpimonth, $tgminmonth, $acminmonth) {
   	    $TG=0;
		$TGC=0;
		$AC=0;
		$ACC=0;
	...
	$mildata1 = "SELECT sum(budgetqty) as "."TGC".$var1." FROM planned WHERE itemid = ".$itemid." and rid=".$rid." AND budgetdate BETWEEN '".
	$tgminmonth."' AND '".$kpimonth."'";
	$mildataresult = mysql_query($mildata1);
	$mildata = mysql_fetch_array($mildataresult);
...
while ($rows = mysql_fetch_array($result)) {
	...
	while ($scalerows = mysql_fetch_array($scaleresult)) {
		...
		mildatamaker($rows['itemid'],$rows['rid'], $var1,$tgminmonth, $acminmonth);
	}	
}
