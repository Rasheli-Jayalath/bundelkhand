...
function doFilter(frm){
	...
	document.location = 'ED.php?' + qString;
}
...
<span style="font-family:Arial, Helvetica, sans-serif; font-size:24px; color:#333; font-weight:bold; padding-left:300px" ><?php echo "ED Dashboard" ;?></span>
</div>
</div>
...
<?php
//$sql = "SELECT ed_format_id, district_id, ws_id, wsrc_id, noffcr_name, noffcr_number, dpr_agn_name, dpr_nom_name, dpr_nom_number, pmc_name, pmc_address, swsmrep_name, swsmrep_number, contr_name, contrnom_name, contrnom_number, nit_number, nit_date, nitcr_capex, nitcr_opex, nitcr_total, contawdcr_capex, contawdcr_opex, contawdcr_total, boqupd_status, dprst_subm_date, dprst_rev_date, dprst_vetiit_date, dprst_apr_date, pmc_cont_date, cont_dpr_date, techbdo_date, finbdo_date, remarks, fps_advance, fps_uptodate_bill, dprcost_cr, efccost_cr, tndbdcost_cr, aggrm_date, intwtp_noc_date, iittovet_date FROM rs_tbl_edformat";
...
<tr style="background:images/table_headingBG.png; padding-bottom:0px; margin-bottom:0px" align="center"><th rowspan="2">EDID</th>
  ...
  <th rowspan="2">NIT Number</th>
  <th colspan="3">NIT Cost</th>
  <th colspan="3">Contract Award Cost</th>
  <th rowspan="2"> BOQ Updating Status  70 LPCD to 55 LPCD</th>
  <th colspan="4">DPR Status 70 LPCD to 55 LPCD</th>
  <th rowspan="2">Date of Contract-PMC</th>
  <th rowspan="2">Date of Contract-DPR</th>
  <th rowspan="2">Technical Bid Opening Date</th>
  <th rowspan="2">Financial Bid Opening Date</th>  
...
<tr align="center">
  ...
  <th>Opex</th>
  <th>Total</th>
  <th>Date of submission of DPR @55 LPCD</th>
  <th>Review by PMC</th>
  <th>Date of Vetting By IIT</th>
  <th>Date of Approval by competent authority</th>
  </tr>
...
<?php if (mysql_num_rows($result) > 0) {
 ...
 $i=1;
  ...
  while($row =mysql_fetch_array($result)) {
   ...
   <td><?php echo $row["nit_number"]; ?></td>
   <td><?php echo $row["nitcr_capex"]; ?></td>
   <td><?php echo $row["nitcr_opex"]; ?></td>
	 <td><?php echo $row["nitcr_total"]; ?></td>
	 <td><?php echo $row["contawdcr_capex"]; ?></td>
	 <td><?php echo $row["contawdcr_opex"]; ?></td>
	 <td><?php echo $row["contawdcr_total"]; ?></td>
	 <td><?php echo $row["boqupd_status"]; ?></td>
	 <td><?php echo $row["dprst_subm_date"]; ?></td>
	 <td><?php echo $row["dprst_rev_date"]; ?></td>
	 <td><?php echo $row["dprst_vetiit_date"]; ?></td>
	 <td><?php echo $row["dprst_apr_date"]; ?></td>
	 <td><?php echo $row["pmc_cont_date"]; ?></td>
	 <td><?php echo $row["cont_dpr_date"]; ?></td>
	 <td><?php echo $row["techbdo_date"]; ?></td>
	 <td><?php echo $row["finbdo_date"]; ?></td>
...
$act_quey="SELECT a.aid, a.itemid, a.startdate, a.enddate, a.rid, a.baseline, a.temp_id, a.ws_id, a.act_type, b.itemname, c.unit, c.quantity FROM activity a inner join maindata b on(a.itemid=b.itemid) inner join baseline c on (a.rid=c.rid) where a.ws_id=".$row["ws_id"]." AND a.act_type=2";
  ...
  echo '</tr><td colspan="50" ><strong>No results</strong></td></tr>';
}
