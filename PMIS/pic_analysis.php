...
if(isset($_REQUEST['sub_comp_id']))
{
$sub_comp_id=$_REQUEST['sub_comp_id'];
}
...
$pdSQLq = "SELECT a.phid, a.pid, a.al_file, a.ph_cap, a.date_p, b.title FROM  project_photos a inner join locations b on(a.ph_cap=b.lid) WHERE a.pid=".$pid;
		...
		if(!empty($_GET['comp_id'])){
			$comp_id = urldecode($_GET['comp_id']);
			$pdSQLq .=" AND comp_id='".$comp_id."'";
		}
...
function doFilter(frm){
	...
	if(frm.sub_comp_id.value != ""){
		qString += '&sub_comp_id=' + escape(frm.sub_comp_id.value);
	}
	if(frm.channel_id.value != ""){
		qString += '&channel_id=' + escape(frm.channel_id.value);
	}
	if(frm.date_p.value != ""){
	...
	document.location = 'pic_analysis.php?' + qString;
}
</script>
<script>
...
<form action="" target="_self" method="post"  enctype="multipart/form-data">
  ...
  <tr><td colspan="2" align="left"><strong><?php echo COMP; ?>:</strong></td></tr>
  <tr>
  <td colspan="2"><select id="comp_id" name="comp_id" onchange="getDates(this.value)" style="width:242px">
  ...
  <tr><td colspan="2" align="left"><strong><?php echo SUB_COMP; ?>:</strong></td></tr>
  <tr>
  <td colspan="2"><select id="sub_comp_id" name="sub_comp_id" onchange="getDates(this.value)" style="width:242px">
     	<option value="0"><?php echo SUB_COMP ?></option>
  		<?php 
		if(isset($comp_id)&&$comp_id!=0)
		{
		$pdSQL1 = "SELECT * FROM  subcomponent_tbl WHERE comp_id=".$comp_id." order by sub_comp_id";
						...
						$i=0;
							  ...
							  { 
							  $i++;?>
  <option value="<?php echo $pdData1["sub_comp_id"];?>" <?php if($sub_comp_id==$pdData1["sub_comp_id"]) {?> selected="selected" <?php }?>><?php echo $pdData1["subcomp_name"];?></option>
   <?php } 
   } }?>
  </select></td>
  </tr>
  <tr><td colspan="2" align="left"><strong><?php echo CHANEL; ?>:</strong></td></tr>
  <tr>
  <td colspan="2"><select id="channel_id" name="channel_id" onchange="getDates(this.value)" style="width:242px">
     	<option value="0"><?php echo CHANEL ?></option>
  		<?php 
		if(isset($sub_comp_id)&&$sub_comp_id!=0)
		{
		$pdSQL = "SELECT * FROM  channel_tbl WHERE sub_comp_id=".$sub_comp_id." order by channel_id";
						 $pdSQLResult = mysql_query($pdSQL);
						$i=0;
							  if(mysql_num_rows($pdSQLResult)>=1)
							  {
							  ...
							  { 
							  $i++;?>
  <option value="<?php echo $pdData["channel_id"];?>" <?php if($channel_id==$pdData["channel_id"]) {?> selected="selected" <?php }?>><?php echo $pdData["channel_name"];?></option>
   <?php } 
   }
		}?>
  </select></td>
...
<table style="width:100%; height:auto; border:1px solid #ccc; border-radius:6px;"  >
 ...
 <?php  
	...
	echo $data_url."thumb/".$result['al_file']; ?>"  border="0" width="150px" height="112px" title="<?php echo $result['ph_cap'];?>"/>
	...
	<img src="images/tag_small.png"  border="0" width="150" height="100" />
			...
			{
			?>
			<tr><td><?php echo NO_RECORD;?></td></tr>
			<?php
			}?>
                              </tbody>
      </table>
