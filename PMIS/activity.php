...
if($item!="")
{
 $sqlgx="Select itemname, parentcd from maindata where stage='Output' and itemid=$item";
...
if($updateBtn !=""){
	...
	$eSql_s = "Select * from maindata where itemid='$txtoutput'";
  	$objDb -> query($eSql_s);
...
function get_output(oc_value) {
		
		
		var strURL="findoutput.php?ot_cm="+oc_value;
			var req = getXMLHTTP();
...
<div id="content">
	  ...
	  <table width="100%" border="0"  align="center" cellpadding="1" cellspacing="1">
		...
		<tr>
			 ...
			 <input  type="text" name="name_strg" value="<?php echo $name_strg; ?>" readonly=""/></td>
			 <?php
			 }
			 else
			 {?>
		...
		<tr>
			 ...
			 <input  type="text" name="name_outcome" value="<?php echo $name_outcome; ?>" readonly=""/></td>
			 <?php
			 }
			 else
			 {?>
              <td >
			  <div id="outcomes">
			  <select name="txtoutcomes" onchange="get_output(this.value)">
			   <option value="">Select</option>
			  <?php $sqlgo="Select * from maindata where stage='Outcome'";
			$resgo=mysql_query($sqlgo);
			while($row3go=mysql_fetch_array($resgo))
			{
			$itemid=$row3go['itemid'];
			if($itemid==$ou_cm)
			{
			$sel="selected='selected'";
			}
			...
			?>
			  <option value="<?php echo $itemid;?>" <?php echo $sel; ?> ><?php echo $row3go['itemname']; ?> </option>
			  <?php
			  }
			  ?>
			  </select>
		...
		<tr>
			 ...
			 {?>
              <td >
			  <div id="output">
			  <select name="txtoutput" onchange="get_activity(this.value)">
			   <option value="">Select</option>
			  <?php $sqlop="Select * from maindata where stage='Output'";
            ...
            <tr>
              <td class="label">&nbsp;</td>
              <td class="label">Activity Code:</td>
			 ...
			 <tr>
              ...
              <td ><input id="txtitemname" name="txtitemname" type="text" value="<?php echo $itemname; ?>"/></td>
			 	...
			 	<tr>
			  <td class="label">&nbsp;</td>
              <td class="label">Activity Weight:</td>
              <td >
...
<input type="text" name="valueitemcode"  id="valueitemcode"  title="Activity Code" placeholder="Activity Code" style="width:100px"    onkeyup="showResult(module.value,this.value,valueitemname.value,valueweight.value,valueisentry.value)"/>
<input type="text" name="valueitemname"  id="valueitemname" title="Activity Name" placeholder="Activity Name" style="width:100px"    onkeyup="showResult(module.value,valueitemcode.value,this.value,valueweight.value,valueisentry.value)"/>
...
<div id="search"></div>
    ...
    <tr bgcolor="#333333" style="text-decoration:inherit; color:#CCC">
     ...
     <th align="center" width="10%"><span class="label">Activity Code</span></th>
	 <th align="center" width="15%"><span class="label">Activity Name</span></th>
...
<strong>
<?php
 $sSQL = " Select * from maindata where stage='Activity' and activitylevel=0";
 $objDb->query($sSQL);
...
<td width="210px" align="right"><?=$activities;?><a href="subactivity.php?subaid=<?php echo $itemid?>&levelid=0"  >Add Subactivity</a></td>
<td width="180px"  ><?=$isentry1;?></td>
<td style="border-bottom:1px solid #cccccc" width="210px" >&nbsp;
 <a href="activity.php?edit=<?php echo $itemid;?>"  ><img src="images/edit.png" width="22" height="22"/></a></td>
