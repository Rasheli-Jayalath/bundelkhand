...
if($valuestage!="")
{
		...
		if($sCondition!="")
		{
		$sCondition.=" AND (stage  like '%".$valuestage."%' and stage='BOQ' and isentry=0) ";
		}
		else
		{
		$sCondition=" (stage  like '%".$valuestage."%' and stage='BOQ' and isentry=0) ";
		}
...
if($valueitemcode!="")
{
		...
		if($sCondition!="")
		{
		$sCondition.=" AND (itemcode  like '%".$valueitemcode."%' and stage='BOQ' and isentry=0) ";
		}
		else
		{
		$sCondition=" (itemcode  like '%".$valueitemcode."%' and stage='BOQ' and isentry=0) ";
		}
...
if($valueitemname!="")
{
		...
		if($sCondition!="")
		{
		$sCondition.=" AND (itemname  like '%".$valueitemname."%' and stage='BOQ' and isentry=0) ";
		}
		else
		{
		$sCondition=" (itemname  like '%".$valueitemname."%' and stage='BOQ' and isentry=0) ";
		}
...
$valueisentry=0;
		...
		if($sCondition!="")
		{
		$sCondition.=" AND (isentry=$valueisentry and stage='BOQ' and isentry=0) ";
		}
		else
		{
		$sCondition=" (isentry=$valueisentry and stage='BOQ' and isentry=0) ";
		}
...
if (strpos($valueweight,'-') === 0) {
	...
	$sCondition.=" AND (weight <= '$expstart' and stage='BOQ' and isentry=0) ";
	}
	else
	{
	$sCondition= " (weight  <= '$expstart' and stage='BOQ' and isentry=0) ";
	}
} 
	...
	if($sCondition!="")
	{
	$sCondition.=" AND (weight between ".$expstart." and ".$expend." and stage='BOQ' and isentry=0) ";
	}
	else
	{
	$sCondition= " (weight between ".$expstart." and ".$expend." and stage='BOQ' and isentry=0) ";
	}
}	
}
 ...
 <div class="with_search">
    ...
    <th></th>
	 <th align="center" width="50%"><strong>Item Name</strong></th>
	  ...
	  <th align="center" width="15%"><span class="label">Isentry</span></th>
...
function AddNewSizeProject<?php echo $itemid; ?>(){
	...
	doAddTr(arrTds, 'tblPrdSizesProject<?php echo $itemid; ?>');
		...
		<td style=" font-size:10px;  color: #000000; background-color: <?php echo $colorr; ?>"><input class="checkbox2" type="checkbox" name="sel_checkbox[]" id="sel_checkbox[]" value="<?=$itemid ?>"   form="reports" onclick="group_checkbox2();">		</td>
