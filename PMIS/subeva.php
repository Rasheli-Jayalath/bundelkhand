...
if($subaid!="")
{
 $sqlgx="Select itemname, parentcd,parentgroup from maindata where stage='EVA' and itemid=$subaid";
...
if($saveBtn != "")
{
  ...
  $eSqls = "Select * from maindata where itemid=".$txtkpi;
  $objDb -> query($eSqls);
 ...
 $sSQL = ("INSERT INTO maindata (parentcd, activitylevel, stage,itemcode, itemname, weight, isentry) VALUES ($txtparentcd,$kpilevel+1,'$txtstage','$txtitemcode', '$txtitemname',$txtweight,$txtisentry)");
	$objDb->execute($sSQL);
...
if($updateBtn !=""){
	 ...
	 $eSql_s = "Select * from maindata where itemid='$txtkpi'";
  	$objDb -> query($eSql_s);
...
while($res_sr=mysql_fetch_array($q_ryr))
{
	...
	$eSql_child = "delete from eva_activity where evaid=$itemid";
    $objDb -> query($eSql_child);
	$eSql_d = "delete from maindata where itemid=$itemid";
    $objDb -> query($eSql_d);
}

header("Location: evadata.php");	
}
