...
function showResult(strmodule,strstage,stritemcode,stritemname,strweight,strisentry) {
  ...
  xmlhttp.open("GET","searcheva.php?module="+strmodule+"&stage="+strstage+"&itemcode="+stritemcode+"&itemname="+stritemname+"&weight="+strweight+"&isentry="+strisentry,true);
  xmlhttp.send();
}
...
var activity=str;
  ...
  xmlhttp1.open("GET","addactivitieseva.php?act="+activity+"&itemid="+itemid,true);
  xmlhttp1.send();
}
...
function edit_data(id,itemid) {
  ...
  xmlhttp1.open("GET","editdataeva.php?aid="+id+"&itemid="+itemid,true);
  xmlhttp1.send();
}

</script>
<script>
function update_data(eaid) {
...
var eaweight = document.getElementById('kaweight').value;
  ...
  xmlhttp4.open("GET","updatedataeva.php?eaid="+eaid+"&itemid="+txtitemid+"&eaweight="+eaweight,true);
  xmlhttp4.send();
}

</script>
<script>
function remove_data(activityid,evaid) {
  if (window.XMLHttpRequest) {
    // code for IE7+, Firefox, Chrome, Opera, Safari
    xmlhttp4=new XMLHttpRequest();
  } else {  // code for IE6, IE5
  ...
  xmlhttp4.onreadystatechange=function() {
    if (xmlhttp4.readyState==4 && xmlhttp4.status==200) {
      document.getElementById("abc"+evaid).innerHTML=xmlhttp4.responseText;
	 // document.getElementById("addnew"+milestoneid).style.display="block";
    }
  }

  xmlhttp4.open("GET","removedataeva.php?activityid="+activityid+"&evaid="+evaid,true);
  xmlhttp4.send();
}
...
function cancel_data(id) {
  ...
  xmlhttp1.open("GET","canceldataeva.php?itemid="+id,true);
  xmlhttp1.send();
}
...
<form name="reports" id="reports"  method="post"  onsubmit="return atleast_onecheckbox(event)" style="display:inline-block; width:100%;margin-top:10px;"> 
		...
		<a class="button" href="javascript:void(null);" onclick="window.open('eva.php', 'Add EVA','width=670px,height=550px,toolbar=0,menubar=0,location=0,status=0,scrollbars=0,resizable=0,left=0,top=0');" >Add EVA Framework</a>
			</div>
...
<input name="submit" type="submit" value="Print List" formaction="reporteva.php" />
<div id="search"></div>
<div id="without_search">
	...
	?>
	  ...
	  ?>
	  <th align="center" width="5%"><strong>Stage</strong></th>
	...
	{
	?>
	   <th align="center" width="20%"><strong>Action  </strong></th>
	   <?php
	   }
	   ?>
...
<?php
		 $sSQL = "SELECT * FROM maindata where stage='EVA' order by parentgroup, parentcd";
...
<tr id="abcd<?php echo $cdlist[$items-1];?>">
		<?php
		$cdsql = "select * from maindata where stage='EVA' and itemid = ".$cdlist[$items-1];
		...
		<?php 
		if($stage=='EVA' && $activitylevel==0)
		{
		$editlink='eva.php';
		$redirect="subeva.php?subaid=$itemid&levelid=$activitylevel";
		$redirect_title="Add SubEVA";
		}
		else if($stage=='EVA' && $activitylevel>0)
		{
		$editlink='subeva.php';
		$redirect="subeva.php?subaid=$itemid&levelid=$activitylevel";
		$redirect_title="Add SubEVA";
		}
		$deletelink='subeva.php';
		  ?>
		...
		?>
		 ...
		 <?php  if($stage=='EVA' && $activitylevel>0) {?>
		...
		<?php
			...
			{
			?>
		 | <a href="<?php echo $deletelink; ?>?del=<?php echo $itemid;?>"   onclick="return confirm('Are you sure you want to delete this EVA and all of its child?')">Delete</a>
		<?php } }else{?>
		...
		<?php
			...
			{
			?>
		| <a href="<?php echo $deletelink; ?>?del=<?php echo $itemid;?>" onclick="return confirm('Are you sure you want to delete this EVA and all of its child?')">Delete</a><?php }} ?>
			 ...
			 ?>
		 <td style=" font-size:10px;  color: #000000; background-color: <?php echo $colorr; ?>" >
		 <a href="logeva.php?trans_id=<?php echo $itemid ; ?>&module=<?php echo $module?>" >Log</a>
	...
	if($cddata['isentry']==1)
	  ...
	  xmlhttp3.open("GET","reloadeva.php?itemid="+id,true);
	  xmlhttp3.send();
