<?php 
if($adata["isentry"]==0&&$data_level_param!="activity"&&$data_level_param!="output"&&$data_level_param!="outcome"&&$data_level_param!="obj")
{
 //echo GetActualQtysSubActLevel($aparentgroup,$aweight ,$afactor);
//echo GetPlannedQtysSubActLevel($aparentgroup,$aweight ,$afactor);
?>
...
$(function () {
            ...
            series: [
		{
                ...
                data: [
				<?php echo GetActualQtysSubActLevel($aparentgroup,$aweight ,$afactor);?>
			...
			,{
                name: 'Planned',
                data: [
				<?php echo GetPlannedQtysSubActLevel($aparentgroup,$aweight ,$afactor);?>
...
<tr bgcolor="000066" style="color:#FFF">
  <td colspan="18" align="left" style="font-size:14px"><strong><?php   echo  $adetail;?></strong></td>
</tr>
<tr>
  <th width="17" height="37" rowspan="2">#</th>
  <th width="145" rowspan="2">Activity</th>
  <th width="15" rowspan="2">Code</th>
  <th width="15" rowspan="2">Weight</th>
  ...
  <th width="59" rowspan="2">Weighted Progress</th>
...
$timeRemainingDiff=0;
$weighted_prog=0;
$total_weighted_prog=0;
?>
<?php if($data_level_id!=0&&$data_level_id!="")
		{
	$reportquery_act ="SELECT * FROM maindata where parentcd=".$data_level_id." order by  itemid ASC";
	...
	$reportquery ="SELECT * FROM activity where itemid=".$reportdata_act["itemid"]." order by  itemid ASC";
 ...
 else
 {
	...
	echo  $planned_progress. "-".$reportdata["baseline"];
	 $planned_perc=$planned_progress/$reportdata["baseline"]*100;
	 $total_work_perc=$reportdata["baseline"]/$reportdata["baseline"]*100;
	 $actual_perc=$till_today_qty/$reportdata["baseline"]*100;
	 
	 if($planned_perc!=0)
	{
	$remaining= $total_work_perc-$actual_perc;
	}
	else
	{
	$remaining=0;
...
<td style="text-align:left;"><?php echo  $act_name;?></td>
<td style="text-align:center;"><?php echo $act_code;?></td>
...
<td style="text-align:right;"><?php echo number_format($total_work_perc,2);?></td>
