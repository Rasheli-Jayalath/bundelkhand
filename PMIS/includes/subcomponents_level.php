...
$(function () {
        $('#container').highcharts({
            ...
            subtitle: {
                text: '<?php echo "(Sub Component Progress ".date('Y',strtotime($lastMonth)).")";?>'
            },
            xAxis: {
                type: 'datetime',
...
<td colspan="30" align="center"><span class="white"><strong><?php echo $pdata["pdetail"];?> (Sub Component Level  <?php echo date('Y',strtotime($lastMonth)); ?>)</strong></span></td>
</tr>
<tr>
...
$ptodate=0;
$ptolast=0;
$reportquery ="SELECT z.itemid,z.itemname, z.itemcode, z.parentcd,z.activitylevel,z.weight, min(b.startdate) as startdate, max(b.enddate) as enddate, sum(b.baseline) as baseline,z.parentgroup,z.activitylevel , b.rid FROM maindata z inner join kpi_activity a on (z.itemid=a.kpiid) inner join activity b on (a.activityid=b.aid) inner join mildata c on (b.itemid=c.itemid AND b.rid=c.rid) where z.parentgroup LIKE '".$aparentgroup."%' Group by z.itemid  order by z.activitylevel";
$i=0;
$progress=0;
$pcurrent=0;
$pprev=0;
...
<tr style="background-color:#9CC;">
  ...
  <td height="20" style="text-align:right;">Accumulative</td>
   <?php
  $sub_com_prog_till_month=getSubComponentCommProgC($lastMonth,$data_level_id, $till_end);
  $sub_com_targ_till_month=getSubComponentCommTargC($lastMonth,$data_level_id,$start);
