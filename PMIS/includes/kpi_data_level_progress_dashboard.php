...
$latest_achieved=0;
$baseline=0;
$reportquery ="SELECT z.itemid,z.itemname, z.itemcode, z.parentcd,z.activitylevel,z.weight, min(b.startdate) as startdate, max(b.enddate) as enddate, sum(b.baseline) as baseline FROM maindata z inner join kpi_activity a on (z.itemid=a.kpiid) inner join activity b on (a.activityid=b.aid) inner join mildata c on (b.itemid=c.itemid AND b.rid=c.rid) where kpiid=".$data_level_id." Group by z.itemid";
$i=0;
$progress=0;
$pcurrent=0;
$pprev=0;
...
while ($reportdata = mysql_fetch_array($reportresult)) 
{
									 ...
									 $scalesql = "SELECT scmonth FROM kpiscale WHERE  scmonth>='".$till_end."' AND scmonth<='".$gend."' order by scid 
									  ASC";
...
<td colspan="30" align="center"><span class="white"><strong><?php echo $pdata["pdetail"];?> (Milestone Level <?php echo date('Y',strtotime($lastMonth)); ?>)</strong></span></td>
</tr>
<tr>
...
$lastMonth = str_replace("-","",$lastMonth);
$reportquery ="SELECT z.itemid,z.itemname, z.itemcode, z.parentcd,z.activitylevel,z.weight, min(b.startdate) as startdate, max(b.enddate) as enddate, sum(b.baseline) as baseline, b.rid FROM maindata z inner join kpi_activity a on (z.itemid=a.kpiid) inner join activity b on (a.activityid=b.aid) inner join mildata c on (b.itemid=c.itemid AND b.rid=c.rid) where kpiid=".$data_level_id." Group by z.itemid";
