...
<tr id="tblHeading">
  <th rowspan="2" style="border-left:1px #000000 solid;"><?php echo SR_NO;?> </th>
<th rowspan="2"> <?php echo CODE;?> </th>
<th rowspan="2"><?php echo DESC;?> </th>
<!--<th colspan="3">As Per Estimate</th>-->
<th colspan="2"> <?php echo AS_PER_BID;?></th>
<th colspan="2"><?php echo PAID_UPTO;?>  <?php if($ipcnodatasec!=0)echo $ipcnodatasec["ipcno"];?></th>
<th colspan="2"><?php echo PAID_IN;?> <?php echo $ipcnodata["ipcno"];?> </th>
<th colspan="2"><?php echo EXC_UPTO;?>  <?php echo $ipcnodata["ipcno"];?> </th>
<th colspan="2"> <?php echo PER_PROG;?></th>
</tr>
<tr id="tblHeading">
						 <?php if($cur_1!="")
						  {?>
						 <th style="width:15%;"><?php echo $cur_1; ?>&nbsp;<?php echo AMNT;?></th>
						<?php }?>
						   <?php if($cur_2!="")
						  {?>
						 <th style="width:15%;"><?php echo $cur_2; ?>&nbsp;<?php echo AMNT;?></th>
						<?php }?>
                           <?php if($cur_3!="")
						  {?>
						 <th style="width:15%;"><?php echo $cur_3; ?>&nbsp;<?php echo AMNT;?></th>
						<?php }?>
                        <?php if($cur_1!="")
						  {?>
						 <th style="width:15%;"><?php echo $cur_1; ?>&nbsp;<?php echo AMNT;?></th>
...
function getProgressAmountD1($boqid)
{
	$ipc_amount1=0;
 if($boqid!=0&&$boqid!='')
 {
    $sql="Select * from boq where boqid=".$boqid;
	 ...
	 $pamountresult3 = mysql_query($sql2)or die(mysql_error());
	 $pdata=mysql_fetch_array($pamountresult3);
	  $ipc_amount1=$data['boq_cur_1_rate']*$data['cur_1_exchrate']*$pdata["ipcqty"];
	 
	 
 return $ipc_amount1;
 }
}
function getProgressAmountD2($boqid)
{
	$ipc_amount2=0;
 if($boqid!=0&&$boqid!='')
 {
    $sql="Select * from boq where boqid=".$boqid;
	 ...
	 $pamountresult3 = mysql_query($sql2)or die(mysql_error());
	 $pdata=mysql_fetch_array($pamountresult3);
	  $ipc_amount2=$data['boq_cur_2_rate']*$data['cur_2_exchrate']*$pdata["ipcqty"];
	 
	 
 return $ipc_amount2;
 }
}
function getProgressAmountD3($boqid)
{
	$ipc_amount3=0;
 if($boqid!=0&&$boqid!='')
 {
    $sql="Select * from boq where boqid=".$boqid;
	 ...
	 $pamountresult3 = mysql_query($sql2)or die(mysql_error());
	 $pdata=mysql_fetch_array($pamountresult3);
	  $ipc_amount3=$data['boq_cur_3_rate']*$data['cur_3_exchrate']*$pdata["ipcqty"];
...
function getThisProgressQTY($boqid,$ipcid)
{
    ...
    if($boqid!=0&&$boqid!="")
    {
	 ...
	 $pamountresult = mysql_query($sql)or die(mysql_error());
	 $data=mysql_fetch_array($pamountresult);
	 if($data["boqid"]!=0&&$data["boqid"]!="" && $ipcid!="")
	 {
	 echo $sql2="SELECT * FROM ipcv  WHERE boqid=".$data["boqid"]." AND ipcid=".$ipcid." GROUP BY boqid";
	 $pamountresult3 = mysql_query($sql2)or die(mysql_error());
	 $pdata=mysql_fetch_array($pamountresult3);
	 $amount=$pdata["ipcqty"];
	 }
...
function getThisProgressQTY1($boqid,$ipcid)
{
	$ipc_amount1=0;
    if($boqid!=0&&$boqid!="")
    {
...
function getThisProgressQTY2($boqid,$ipcid)
{
	$ipc_amount2=0;
    if($boqid!=0&&$boqid!="")
    {
...
function getThisProgressQTY3($boqid,$ipcid)
{
	$ipc_amount3=0;
    if($boqid!=0&&$boqid!="")
    {
...
function getProgressAmountIPC1($itemid,$ipcid)
{
	$ipc_amount1=0;
	$i=1;
 if($itemid!=0&&$itemid!=0&&$itemid!='')
...
function getProgressAmountIPC2($itemid,$ipcid)
{
	$ipc_amount2=0;
	$i=1;
 if($itemid!=0&&$itemid!=0&&$itemid!='')
...
function getProgressAmountIPC3($itemid,$ipcid)
{
	$ipc_amount3=0;
	$i=1;
 if($itemid!=0&&$itemid!=0&&$itemid!='')
...
function getProgressAmountIPCBOQ($boqid,$ipcid)
{
	$sum=0;
	$amount=0;
	$i=1;
 if($boqid!=0&&$boqid!=0&&$boqid!='')
 {
	 $sql="Select * from boq where boqid=".$boqid;
	 $pamountresult = mysql_query($sql)or die(mysql_error());
	 while($data=mysql_fetch_array($pamountresult))
...
function getProgressAmountIPCBOQ1($boqid,$ipcid)
{
	$ipc_amount1=0;
	$i=1;
 if($boqid!=0&&$boqid!=0&&$boqid!='')
...
function getProgressAmountIPCBOQ2($boqid,$ipcid)
{
	$ipc_amount2=0;
	$i=1;
 if($boqid!=0&&$boqid!=0&&$boqid!='')
...
function getProgressAmountIPCBOQ3($boqid,$ipcid)
{
	$ipc_amount3=0;
	$i=1;
 if($boqid!=0&&$boqid!=0&&$boqid!='')
...
function getProgressAmountBOQ($boqid)
{
	$sum=0;
	$i=1;
 if($boqid!=0&&$boqid!=0&&$boqid!='')
...
function getProgressAmountBOQ1($boqid)
{
	$ipc_amount1=0;
	$i=1;
 if($boqid!=0&&$boqid!=0&&$boqid!='')
...
function getProgressAmountBOQ2($boqid)
{
	$ipc_amount2=0;
	$i=1;
 if($boqid!=0&&$boqid!=0&&$boqid!='')
...
function getProgressAmountBOQ3($boqid)
{
	$ipc_amount3=0;
	$i=1;
 if($boqid!=0&&$boqid!=0&&$boqid!='')
...
function getProgressAmount1($itemid)
{
	$sum=0;
	$i=1;
	$ipc_amount1=0;
...
function getProgressAmount2($itemid)
{
	$sum=0;
	$i=1;
	$ipc_amount2=0;
...
function getProgressAmount3($itemid)
{
	$sum=0;
	$i=1;
	$ipc_amount3=0;
...
{
	$sum=0;
 if($itemid!=0&&$itemid!=0&&$itemid!=""&&$ipcid!="")
 {
	 $sql="Select * from boq where itemid=".$itemid;
	 $pamountresult = mysql_query($sql)or die(mysql_error());
	 while($data=mysql_fetch_array($pamountresult))
...
function getThisProgressAmount1($itemid,$ipcid)
{
	$this_ipc_amount1=0;
 if($itemid!=0&&$itemid!=0&&$itemid!=""&&$ipcid!="")
 {
	 ...
	 $this_ipc_amount1+=$data['boq_cur_1_rate']*$data['cur_1_exchrate']*$pdata["ipcqty"];
	 
	 }
 return $this_ipc_amount1;
 }
}
function getThisProgressAmount2($itemid,$ipcid)
{
	$this_ipc_amount2=0;
 if($itemid!=0&&$itemid!=0&&$itemid!=""&&$ipcid!="")
 {
	 ...
	 $this_ipc_amount2+=$data['boq_cur_2_rate']*$data['cur_2_exchrate']*$pdata["ipcqty"];
	 
	 }
 return $this_ipc_amount2;
 }
}
function getThisProgressAmount3($itemid,$ipcid)
{
	$this_ipc_amount3=0;
 if($itemid!=0&&$itemid!=0&&$itemid!=""&&$ipcid!="")
 {
	 ...
	 $this_ipc_amount3+=$data['boq_cur_3_rate']*$data['cur_3_exchrate']*$pdata["ipcqty"];
	 
	 }
 return $this_ipc_amount3;
 }
}
function getThisProgressAmountBOQ($boqid,$ipcid)
{
	$sum=0;
 if($boqid!=0&&$boqid!=0&&$boqid!=""&&$ipcid!="")
 {
	 $sql="Select * from boq where boqid=".$boqid;
	 $pamountresult = mysql_query($sql)or die(mysql_error());
	 while($data=mysql_fetch_array($pamountresult))
...
function getThisProgressAmountBOQ1($boqid,$ipcid)
{
	$this_ipc_amount1=0;
 if($boqid!=0&&$boqid!=0&&$boqid!=""&&$ipcid!="")
 {
...
function getThisProgressAmountBOQ2($boqid,$ipcid)
{
	$this_ipc_amount2=0;
 if($boqid!=0&&$boqid!=0&&$boqid!=""&&$ipcid!="")
 {
...
function getThisProgressAmountBOQ3($boqid,$ipcid)
{
	$this_ipc_amount3=0;
 if($boqid!=0&&$boqid!=0&&$boqid!=""&&$ipcid!="")
 {
...
if($str_length_id<6 && $reportdata_act['boqid']!=0 && $reportdata_act['boqid']!="")
	{
    $todate_amount1=getProgressAmountD1($reportdata_act_sub['boqid']);
	$todate_amount2=getProgressAmountD2($reportdata_act_sub['boqid']);
	$todate_amount3=getProgressAmountD3($reportdata_act_sub['boqid']);
	}
    else
	{
     $todate_amount1=getProgressAmount1($reportdata_act_sub['itemid']);
	 $todate_amount2=getProgressAmount2($reportdata_act_sub['itemid']);
	 $todate_amount3=getProgressAmount3($reportdata_act_sub['itemid']);
	}

if($str_length_id<6 && $reportdata_act['boqid']!=0 && $reportdata_act['boqid']!="")
	{
$this_amount1=getThisProgressQTY1($reportdata_act_sub['boqid'],$ipcid);
$this_amount2=getThisProgressQTY2($reportdata_act_sub['boqid'],$ipcid);
$this_amount3=getThisProgressQTY3($reportdata_act_sub['boqid'],$ipcid);

	}
	else
	{
$this_amount1=getThisProgressAmount1($reportdata_act_sub['itemid'],$ipcid);
$this_amount2=getThisProgressAmount2($reportdata_act_sub['itemid'],$ipcid);
$this_amount3=getThisProgressAmount3($reportdata_act_sub['itemid'],$ipcid);
	}
...
if($str_length_id<6 && $reportdata_act['boqid']!=0 && $reportdata_act['boqid']!="")
	{
    $todate_amount1=getProgressAmountBOQ1($reportdata_act_sub['boqid']);
	$todate_amount2=getProgressAmountBOQ2($reportdata_act_sub['boqid']);
	$todate_amount3=getProgressAmountBOQ3($reportdata_act_sub['boqid']);
	}
    else
	{
     $todate_amount1=getProgressAmount1($reportdata_act_sub['itemid']);
...
if($str_length_id<6 && $reportdata_act['boqid']!=0 && $reportdata_act['boqid']!="")
	{
	$this_amount1=getThisProgressAmountBOQ1($reportdata_act_sub['boqid'],$ipcid);
	$this_amount2=getThisProgressAmountBOQ2($reportdata_act_sub['boqid'],$ipcid);
	$this_amount3=getThisProgressAmountBOQ3($reportdata_act_sub['boqid'],$ipcid);
	}
	else
	{
	$this_amount1=getThisProgressAmount1($reportdata_act_sub['itemid'],$ipcid);
...
if($str_length_id<6 && $reportdata_act['boqid']!=0 && $reportdata_act['boqid']!="")
	{
	   $ipc_amount1=getProgressAmountBOQ1($reportdata_act['boqid']);
	   $ipc_amount2=getProgressAmountBOQ2($reportdata_act['boqid']);
	   $ipc_amount3=getProgressAmountBOQ3($reportdata_act['boqid']);
	   $todate_amount1=$ipc_amount1;
	   $todate_amount2=$ipc_amount2;
	   $todate_amount3=$ipc_amount3;
	}
    else
	{
		 
      $ipc_amount1=getProgressAmount1($reportdata_act['itemid']);
	  $ipc_amount2=getProgressAmount2($reportdata_act['itemid']);
	  $ipc_amount3=getProgressAmount3($reportdata_act['itemid']);
	  $todate_amount1=$ipc_amount1;
	  $todate_amount2=$ipc_amount2;
	  $todate_amount3=$ipc_amount3;
	...
	if($str_length_id<6 && $reportdata_act['boqid']!=0 && $reportdata_act['boqid']!="")
	{
	$this_amount1=getThisProgressAmountBOQ1($reportdata_act['boqid'],$ipcid);
	$this_amount2=getThisProgressAmountBOQ2($reportdata_act['boqid'],$ipcid);
	$this_amount3=getThisProgressAmountBOQ3($reportdata_act['boqid'],$ipcid);
	}
	else
	{
	$this_amount1=getThisProgressAmount1($reportdata_act['itemid'],$ipcid);
	$this_amount2=getThisProgressAmount2($reportdata_act['itemid'],$ipcid);
	$this_amount3=getThisProgressAmount3($reportdata_act['itemid'],$ipcid);
	}
...
?>
<?php if($reportdata_act['isentry']==1)
{ 
$reportquery_boq="SELECT b.boqid, b.itemid, b.boqcode, b.boqitem, b.boqunit, b.boq_base_currency, b.boqqty, b.boq_cur_1, b.cur_1_exchrate, b.boq_cur_1_rate, b.boq_cur_2, b.cur_2_exchrate, b.boq_cur_2_rate, b.boq_cur_3, b.cur_3_exchrate, b.boq_cur_3_rate, (b.boqqty* b.boq_cur_1_rate*b.cur_1_exchrate) as contract_amount_1, (b.boqqty* b.boq_cur_2_rate*b.cur_2_exchrate) as contract_amount_2, (b.boqqty* b.boq_cur_3_rate*b.cur_3_exchrate) as contract_amount_3, (b.boqqty* b.boq_cur_1_rate*b.cur_1_exchrate+b.boqqty* b.boq_cur_2_rate*b.cur_2_exchrate+b.boqqty* b.boq_cur_3_rate*b.cur_3_exchrate) as total_amount FROM boq b where b.itemid='".$reportdata_act['itemid']."'";
$reportresult_actboq = mysql_query($reportquery_boq)or die(mysql_error());
while ($reportdata_actboq = mysql_fetch_array($reportresult_actboq)) 
{
$contract_amount1+=$reportdata_actboq['contract_amount_1'];
$contract_amount2+=$reportdata_actboq['contract_amount_2'];
$contract_amount3+=$reportdata_actboq['contract_amount_3'];	

	   $ipc_amount1=getProgressAmountBOQ1($reportdata_actboq['boqid']);
	   $ipc_amount2=getProgressAmountBOQ2($reportdata_actboq['boqid']);
	   $ipc_amount3=getProgressAmountBOQ3($reportdata_actboq['boqid']);
	   $todate_amount1=$ipc_amount1;
	   $todate_amount2=$ipc_amount2;
	   $todate_amount3=$ipc_amount3;
	   $this_amount1=getThisProgressAmountBOQ1($reportdata_actboq['boqid'],$ipcid);
	   $this_amount2=getThisProgressAmountBOQ2($reportdata_actboq['boqid'],$ipcid);
	   $this_amount3=getThisProgressAmountBOQ3($reportdata_actboq['boqid'],$ipcid);
...
<td style="text-align:center;"><?php 
	...
	echo $reportdata_actboq['boqcode'];
?></td>
<td style="text-align:left;"><?php   
	
		 echo $reportdata_actboq['boqitem']; 
	?></td>

<?php if($cur_1!="") {?>
<td style="text-align:right;"><?php  echo number_format ($reportdata_actboq['contract_amount_1'],2) ; ?></td>
<?php }?>
<?php if($cur_2!="")
						  {?>
<td style="text-align:right;"><?php  echo number_format ($reportdata_actboq['contract_amount_2'],2) ; ?></td>
<?php }?>
<?php if($cur_3!="")
						  {?>
<td style="text-align:right;"><?php  echo number_format ($reportdata_actboq['contract_amount_3'],2) ; ?></td>
<?php }?>
<?php if($cur_1!="") {?>
...
if($str_length_id<6 && $itemid!=0 && $itemid!="")
	{
	echo $reportdata_act['boqcode'];
}
else 
{
	echo $reportdata_act['itemcode'];
}?></td>
<td style="text-align:left;"><?php   
	if($str_length_id<6 && $itemid!=0 && $itemid!="")
	{
		 echo $reportdata_act['boqitem']; 
	}
	else 
	{
		echo $reportdata_act['itemname'];
	}?></td>
...
<tr  id="title">
  <td colspan="201" align="center" style="background-color:#000066"><strong><span style="color:#FFF"><?php echo IPC_WISE;?></span></strong></td>
 </tr>
<tr id="tblHeading">
  <th rowspan="2"><?php echo SR_NO;?></th>
<th rowspan="2"><?php echo CODE;?></th>
<th rowspan="2"><?php echo DESC;?> </th>
<th colspan="2"> <?php echo AS_PER_BID;?></th>
...
if($actlevel==0)
{
	if($str_length_id<6 && $reportdata_act_sub['boqid']!=0 && $reportdata_act_sub['boqid']!="")
	{
$boqid=" AND b.boqid=" .$reportdata_act_sub['boqid'];
	}
	else
	{
$boqid=" AND b.itemid=" .$reportdata_act_sub['boqid']." Group by b.itemid";
	}
...
if($str_length_id<6 && $reportdata_act['boqid']!=0 && $reportdata_act['boqid']!="")
	{
$boqid=" AND b.boqid=" .$reportdata_act['boqid'];
	}
	else
	{
$boqid=" AND b.itemid=" .$reportdata_act['itemid']." Group by b.itemid";
	}	
}
...
if($str_length_id<6 && $reportdata_act['boqid']!=0 && $reportdata_act['boqid']!="")
	{
     ...
     $ipc_amount1=getProgressAmountIPCBOQ1($rows2['boqid'],$rows['ipcid']);
	 $ipc_amount2=getProgressAmountIPCBOQ2($rows2['boqid'],$rows['ipcid']);
	 $ipc_amount3=getProgressAmountIPCBOQ3($rows2['boqid'],$rows['ipcid']);
	   $todate_amount1=$ipc_amount1;
	   $todate_amount2=$ipc_amount2;
	   $todate_amount3=$ipc_amount3;
	...
	{
 $todate_amount1=getProgressAmountIPC1($rows2['itemid'],$rows['ipcid']);
  $todate_amount2=getProgressAmountIPC2($rows2['itemid'],$rows['ipcid']);
  $todate_amount3=getProgressAmountIPC3($rows2['itemid'],$rows['ipcid']);
