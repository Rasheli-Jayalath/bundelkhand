...
{
$reportquery ="SELECT max(progressdate) as actual_finish_date FROM progress WHERE itemid=".$aid;
$reportresult = mysql_query($reportquery);
if($reportresult!=0)
{
$reportdata = mysql_fetch_array($reportresult);
...
{
$reportquery="SELECT max(b.progressdate) as actual_finish_date from maindata a inner join progress b on(a.itemid=b.itemid) where a.parentcd=".$aid ;
$reportresult = mysql_query($reportquery);
if($reportresult!=0)
{
$reportdata = mysql_fetch_array($reportresult);
...
while($reportdata = mysql_fetch_array($reportresult))
{
	...
	$total_till_today_qty= $reportdata["total_till_today_qty"];
	$actual_prog=($total_till_today_qty/$baseline)*100*$weight/100;
	//echo "<br/>";
	$total_actual_prog +=$actual_prog;
}
$actual_prog_perc=$total_actual_prog/100;
}
else
{
	$actual_prog_perc=0;
...
$planned_prog_perc=$total_planned_prog/100;
}
else
{
	$planned_prog_perc=0;
...
function GetPlannedQtysDataLevel($aid,$current_date,$planned_start_date)
{
 ...
 $reportquery ="SELECT * FROM planned WHERE itemid=".$aid. " AND budgetdate>='".$planned_start_date."' AND budgetdate<='".$current_date."' "."  group by itemid, budgetdate order by itemid,budgetdate ASC";
				$reportresult = mysql_query($reportquery);
				if($reportresult!=0)
				{
				$num=mysql_num_rows($reportresult);
...
function GetPlannedQtysSubActLevel($aid,$aweight)
{
	...
	// END Plan Scale
	 $dates=array();
				$dates= dateRange($reportdata_scale['startdate'], $reportdata_scale['enddate']);
				$num=sizeof($dates);
				//print_r($dates);
				  $ii=0;
				...
				$ii++;
					  ...
					  $reportquery ="SELECT * FROM activity a inner join maindata b on(a.itemid=b.itemid) WHERE b.parentcd=".$aid ." order by  
					  a.itemid ASC";
					  $reportresult = mysql_query($reportquery);			
					  while ($reportdata = mysql_fetch_array($reportresult)) {
						
						   $planned_progress=PlannedProgressG($reportdata['itemid'],$scale_date);
						   $planned_perc=$planned_progress/$reportdata["baseline"]*100*$reportdata["weight"]/100;
						   $total_planned_perc +=$planned_perc;
						   
					}
					  $planned=$total_planned_perc*$aweight/100;
...
function GetActualQtysSubActLevel($aid,$aweight)
{
	// Get Plan Scale
$scale_query="Select max(progressdate) as enddate , min(progressdate) as startdate from progress where itemid IN (select b.itemid from (select a.startdate, a.enddate, a.baseline ,a.itemid From activity a where itemid IN (SELECT itemid FROM maindata WHERE parentcd=".$aid." AND isentry=1 GROUP BY activitylevel, parentcd ORDER BY maindata.activitylevel)) b)" ;
	$reportresult_scale = mysql_query($scale_query);
	$reportdata_scale=mysql_fetch_array($reportresult_scale);
	// END Plan Scale
	 $dates=array();
				...
				$ii++;
					  ...
					  while ($reportdata = mysql_fetch_array($reportresult)) {
						   ...
						   $actual_progress=ActualProgressG($reportdata['itemid'],$scale_date);
						   
						   $actual_perc=$actual_progress/$reportdata["baseline"]*100*$reportdata["weight"]/100;
						   $total_actual_perc +=$actual_perc;
						   
					}
					 $actual=$total_actual_perc*$aweight/100;
...
function GetPlannedQtysMainActLevel($aparentgroup,$aweight)
{
	...
	// END Plan Scale
	 $dates=array();
				...
				$ii++;
					  ...
					  $vmonth_days=cal_days_in_month(CAL_GREGORIAN,$vmonth,$vyear);
				      $scale_date=$vyear."-".$vmonth."-".$vmonth_days;
					  $reportquery ="Select * from (select a.startdate, a.enddate, a.baseline ,a.itemid From activity a where itemid IN (SELECT itemid FROM maindata WHERE parentgroup LIKE '".$aparentgroup."%' AND isentry=1 GROUP BY activitylevel, parentcd ORDER BY maindata.activitylevel)) b";
					
					  $reportresult = mysql_query($reportquery);			
					  while ($reportdata = mysql_fetch_array($reportresult)) {
						
						  $planned_progress=PlannedProgressG($reportdata['itemid'],$scale_date);
						  $item_weight=GetWeight($reportdata['itemid']);
						  $item_parent_weight=GetParentWeight($reportdata['itemid']);
						  $planned_perc=$planned_progress/$reportdata["baseline"]*100*$item_weight/100*$item_parent_weight/100;
						  $total_planned_perc +=$planned_perc;
...
function GetActualQtysMainActLevel($aparentgroup,$aweight)
{
	// Get Plan Scale
	 ...
	 $dates=array();
				...
				$ii++;
					  ...
					  while ($reportdata = mysql_fetch_array($reportresult)) {
						   ...
						   $item_weight=GetWeight($reportdata['itemid']);
						  $item_parent_weight=GetParentWeight($reportdata['itemid']);
						   $actual_perc=$actual_progress/$reportdata["baseline"]*100*$item_weight/100*$item_parent_weight/100;
						   $total_actual_perc +=$actual_perc;
	 ...
	 $dates=array();
				...
				$ii++;
					  ...
					  while ($reportdata = mysql_fetch_array($reportresult)) {
						  ...
						  $item_weight=GetWeight($reportdata['itemid']);
						  $item_parent_weight=GetParentWeight($reportdata['itemid']);
						  $item_parent_group=GetParentGroup($reportdata['itemid']);
						  $item_parent_group_sub=substr($item_parent_group,0,15);
						  $item_main_parent_weight=GetMainParentWeight($item_parent_group_sub);
						  $planned_perc=$planned_progress/$reportdata["baseline"]*100*$item_weight/100*$item_parent_weight/100*$item_main_parent_weight/100;
						  $total_planned_perc +=$planned_perc;
				...
				$ii++;
					  ...
					  while ($reportdata = mysql_fetch_array($reportresult)) {
					   ...
					   $item_parent_group=GetParentGroup($reportdata['itemid']);
						   ...
						   $actual_perc=$actual_progress/$reportdata["baseline"]*100*$item_weight/100*$item_parent_weight/100*$item_main_parent_weight/100;
						   $total_actual_perc +=$actual_perc;
