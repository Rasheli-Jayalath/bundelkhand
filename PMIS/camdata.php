...
function showResult(strmodule,strstage,stritemcode,stritemname,strweight,strisentry) {
  ...
  xmlhttp.open("GET","searchcam.php?module="+strmodule+"&stage="+strstage+"&itemcode="+stritemcode+"&itemname="+stritemname+"&weight="+strweight+"&isentry="+strisentry,true);
  xmlhttp.send();
}
...
var activity=str;
  ...
  xmlhttp1.open("GET","addactivitiescam.php?act="+activity+"&itemid="+itemid,true);
  xmlhttp1.send();
}
...
function edit_data(id,itemid) {
  ...
  xmlhttp1.open("GET","editdatacam.php?aid="+id+"&itemid="+itemid,true);
  xmlhttp1.send();
}

</script>
<script>
function update_data(camid) {
...
var caweight = document.getElementById('caweight').value;
  ...
  xmlhttp4.open("GET","updatedatacam.php?camid="+camid+"&itemid="+txtitemid+"&caweight="+caweight,true);
  xmlhttp4.send();
}

</script>
<script>
function remove_data(activityid,caid) {
  if (window.XMLHttpRequest) {
    // code for IE7+, Firefox, Chrome, Opera, Safari
    xmlhttp4=new XMLHttpRequest();
  } else {  // code for IE6, IE5
  ...
  xmlhttp4.onreadystatechange=function() {
    if (xmlhttp4.readyState==4 && xmlhttp4.status==200) {
      document.getElementById("abc"+caid).innerHTML=xmlhttp4.responseText;
	 // document.getElementById("addnew"+milestoneid).style.display="block";
    }
  }

  xmlhttp4.open("GET","removedatacam.php?activityid="+activityid+"&caid="+caid,true);
  xmlhttp4.send();
}
...
function cancel_data(id) {
  ...
  xmlhttp1.open("GET","canceldatacam.php?itemid="+id,true);
  xmlhttp1.send();
}
...
<form name="reports" id="reports"  method="post"   style="display:inline-block; width:100%;margin-top:10px;"> 
		...
		<?php  if($camentry_flag==1 || $camadm_flag==1)
	{
	?>
        <a class="button" href="javascript:void(null);" onclick="window.open('strategic_goal_cam.php', 'Add CAM Strategic Goal','width=670px,height=550px,toolbar=0,menubar=0,location=0,status=0,scrollbars=0,resizable=0,left=0,top=0');" >Add CAM Strategic Goal</a>
		 <?php
		 }
		else
		{
		?>
		<a href="javascript:void(0);" style="opacity: 0.5;" class="button" >Add CAM Strategic Goal</a>
		<?php
		}
		?>
		
<!--		 <a class="button" href="javascript:void(null);" onclick="window.open('outcome_cam.php', 'Add CAM Outcome','width=670px,height=550px,toolbar=0,menubar=0,location=0,status=0,scrollbars=0,resizable=0,left=0,top=0');" >Add CAM Outcome</a>
		  <a class="button" href="javascript:void(null);" onclick="window.open('output_cam.php', 'Add CAM Output','width=670px,height=550px,toolbar=0,menubar=0,location=0,status=0,scrollbars=0,resizable=0,left=0,top=0');" >Add CAM Output</a>
		 
		<a class="button" href="javascript:void(null);" onclick="window.open('cam.php', 'Add CAM','width=670px,height=550px,toolbar=0,menubar=0,location=0,status=0,scrollbars=0,resizable=0,left=0,top=0');" >Add CAM</a>-->
...
<input name="submit" type="submit" value="Print List" formaction="reportcam.php"/>
<div id="search"></div>
...
<?php
		 $sSQL = "SELECT * FROM maindata where stage='CAM' order by parentgroup, parentcd";
...
?>
		...
		<?php
		$cdsql = "select * from maindata where stage='CAM' and itemid = ".$cdlist[$items-1];
		...
		<?php 
		if($stage=='CAM' && $activitylevel>3)
		{
		$editlink='subcam.php';
		$redirect="subcam.php?subaid=$itemid&levelid=$activitylevel";
		$redirect_title="Add SubCAM";
		}
		else if($stage=='CAM' && $activitylevel==0)
		{
		$editlink='strategic_goal_cam.php';
		$redirect="outcome_cam.php?item=$itemid";
		$redirect_title="Add Outcome";
		}
		elseif($stage=='CAM' && $activitylevel==1)
		{
		$editlink='outcome_cam.php';
		$redirect="output_cam.php?item=$itemid";
		$redirect_title="Add Output";
		}
		elseif($stage=='CAM' && $activitylevel==2)
		{
		$editlink='output_cam.php';
		$redirect="cam.php?item=$itemid";
		$redirect_title="Add CAM";
		}
		else if($stage=='CAM' && $activitylevel=3)
		{
		$editlink='cam.php';
		$redirect="subcam.php?subaid=$itemid&levelid=$activitylevel";
		$redirect_title="Add SubCAM";
	...
	?>
	  ...
	  if($camadm_flag==1)
		...
		| <a href="<?php echo $deletelink; ?>?del=<?php echo $itemid;?>"   onclick="return confirm('Are you sure you want to delete this Critical Activity  and all of its child?')">Delete</a>
		<?php }}else{?>
	...
	{
	?>
		| <a href="<?php echo $deletelink; ?>?del=<?php echo $itemid;?>" onclick="return confirm('Are you sure you want to delete this Critical Activity and all of its child?')">Delete</a><?php } } ?>
		...
		?>
		 <td style=" font-size:10px;  color: #000000; background-color: <?php echo $colorr; ?>" >
		 <a href="logcam.php?trans_id=<?php echo $itemid ; ?>&module=<?php echo $module?>" >Log</a>
...
if (window.XMLHttpRequest) {
  ...
  xmlhttp3.open("GET","reloadcam.php?itemid="+id,true);
  xmlhttp3.send();
