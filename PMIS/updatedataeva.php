...
@require_once("get_url.php");
 ...
 $sqlr="Select rid from resources where schedulecode='$secheduleid'";
 ...
 $sSQL5 = "UPDATE eva_activity SET
		eaweight = $eaweight
		where eaid=$eaid and evaid=$itemid";
	$objDb->execute($sSQL5);
	$msg="Updated!";
...
<?php 
$sql_b="SELECT a.eaid, b.aid,b.itemid,b.code,b.secheduleid,b.startdate,b.enddate,b.actualstartdate,b.actualenddate, b.aorder, b.baseline, b.remarks, b.rid FROM `eva_activity` a inner join activity b on (a.activityid=b.aid) where a.evaid=".$itemid; 
/*$sql_d="Select * from kpi_activity where milestoneid=$itemid";*/
			...
			<td><?=$row3_b['code'];?></td>
			<?php
			$sql_r="Select boqcode from resources where rid=".$row3_b['rid'];
			$res_r=mysql_query($sql_r);
			$row3_r=mysql_fetch_array($res_r);
			?>			
			<td><?=$row3_r['boqcode']." : ".$row3_b['secheduleid'];?></td>
			...
			<?php
			$sqlga="Select * from eva_activity where eaid=".$eaid;
