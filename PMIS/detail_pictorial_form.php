...
{ 
	...
	copy($_FILES['al_file']['tmp_name'],"temp/".$_FILES['al_file']['name']);	
	include("imageResize.php");
	$imagelink2 = "temp/".$_FILES['al_file']['name'];
	list($widtho, $heighto, $typeo, $attro) = getimagesize($imagelink2);
	$flink = imageResize($_FILES['al_file']['name'],$file_name,2500,300,$file_path);
	...
	Values(".$pid.", '".$flink."', '".$ph_cap."', '".$date_p."' )");
	if ($sql_pro == TRUE) {
...
$date_p=date("Y-m-d",strtotime($_REQUEST['date_p']));
	...
	include("imageResize.php");
	/*$imagelink2 = "temp/".$_FILES['al_file']['name'];
 getimagesize($imagelink2);
	
	list($widtho, $heighto, $typeo, $attro) = getimagesize($imagelink2);
	echo $widtho;*/
	$flink = imageResize($_FILES['al_file']['name'],$file_name,2500,300,$file_path);
	$flink=$file_name.".".$flink;
    $sql_pro="UPDATE  project_photos SET ph_cap='$ph_cap', al_file='$flink' ,date_p='$date_p' where phid=$phid";
