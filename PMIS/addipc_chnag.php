...
<?php
		echo  $sSQL = "SELECT * FROM maindata where stage='BOQ' and isentry=1  order by parentgroup, parentcd";
...
?>
		...
		<?php
	echo	$cdsql = "select * from maindata where stage='BOQ' and itemid = ".$cdlist[$items-1];
...
function AddNewSizeProject<?php echo $itemid; ?>(){
	...
	doAddTr(arrTds, 'tblPrdSizesProject<?php echo $itemid; ?>');
		...
		<td colspan="6">
			 <?php
		/*echo $cdsql_a = "select * from maindata where parentcd = '$cddata[itemid]' and isentry=1 and stage='BOQ'";
