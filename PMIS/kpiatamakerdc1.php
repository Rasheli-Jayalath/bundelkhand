...
//echo "Data is Making Process is complete";
	...
	$actual_perc=0;
	$prev=0;
    $bsql = "SELECT * FROM `kpidata` where isentry=1 ";
	...
	while($lrows = mysql_fetch_array($bresult))
	{
		...
		$totalstring=($kpi_level*4)+($kpi_levels-1);
		$fkpi_levels=$kpi_levels-2;
		for($i=0;$i<=$fkpi_levels;$i++)
		{
		$kpi_levels_c=$kpi_levels-$fkpi_levels;
		$totalstring=$totalstring-5; //(2,3,4)
		$subsrt=substr($lrows["parentgroup"],0,$totalstring);
		$current=$par_arr[$kpi_levels];
		if($current!=$prev)
		{
		echo $llsql= "INSERT INTO kpi_top_level_report( `kpiid`, `scid`, `kpi_plan_per`, `kpi_act_per`) (SELECT kpiid, scid, sum(kpi_comm_planned/baseline*weight/100) as kpi_plan_per , sum(kpi_comm_actual/baseline*weight/100) as kpi_act_per FROM `kpi_base_level_report` where parentgroup LIKE '".$subsrt."%' group by  scid ORDER BY `kpiid`,scid ASC)";
