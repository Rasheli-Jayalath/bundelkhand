	...
	*/
	public function __construct(){
		...
		if($_SESSION['is_login'] == true){
			...
			//KAPAL
				$this->ne_kapal 			= $_SESSION['ne_kapal'];
			$this->ne_kapaladm 		= $_SESSION['ne_kapaladm'];
			$this->ne_kapalentry 	= $_SESSION['ne_kapalentry'];
		}
	}
	...
	*/
	public function setAdminLogin(){
		...
		if($this->isPropertySet("gmcentry", "V"))
			...
			///KAPAL
				if($this->isPropertySet("kapal", "V"))
			$_SESSION['ne_kapal'] 		= $this->getProperty("kapal");
		if($this->isPropertySet("kapaladm", "V"))
			$_SESSION['ne_kapaladm'] 		= $this->getProperty("kapaladm");
		if($this->isPropertySet("kapalentry", "V"))
			$_SESSION['ne_kapalentry'] 		= $this->getProperty("kapalentry");
	}
	...
	*/
	public function setLogout(){
		unset(
				...
				//KAPAL
				$_SESSION['ne_kapal'],
				$_SESSION['ne_kapaladm'],
				$_SESSION['ne_kapalentry']
			);
	}
	...
	public function actAdminUser($mode){
		...
		switch($mode){
			case "I":
				...
				$Sql .= ",";
				$Sql .= $this->isPropertySet("kapal", "V") ?$this->getProperty("kapal") : "0";
				$Sql .= ",";
				$Sql .= $this->isPropertySet("kapaladm", "V") ?$this->getProperty("kapaladm") : "0";
				$Sql .= ",";
				$Sql .= $this->isPropertySet("kapalentry", "V") ? $this->getProperty("kapalentry") : "0";		
			...
			case "U":
				...
				if($this->isPropertySet("kapal", "K")){
					$Sql .= "$con kapal='" . $this->getProperty("kapal") . "'";
					
				}if($this->isPropertySet("kapaladm", "K")){
					$Sql .= "$con kapaladm='" . $this->getProperty("kapaladm") . "'";
					
				}if($this->isPropertySet("kapalentry", "K")){
					$Sql .= "$con kapalentry='" . $this->getProperty("kapalentry") . "'";
