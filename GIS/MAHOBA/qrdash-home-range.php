...
<?php 
$query = "SELECT code,  count(label_f) as total from dgps_survey_data where  component_name='$componentName' and channel_id='".$channel_id."' and chainage_id BETWEEN ".$from_kmpost. " AND ".$to_kmpost." group by code";
//echo $query;
$result=mysql_query($query);
 ...
 while($row = mysql_fetch_assoc($result)) 
 {
	...
	$query_d = 'SELECT label_f from dgps_survey_data where  component_name="'.$componentName.'" and code="'.$dgps_code.'" limit 0,1';
		$result_d=mysql_query($query_d);
		$row_d = mysql_fetch_assoc($result_d);
		$dgps_detail=$row_d['label_f'];
		
	 $m=$m+1;
?>
<tr>
 	<td style="height:1px; padding:; line-height:0.4em;"><?php 
	echo $row_d['label_f'];?></td>
    <td style="text-align:center; height:1px; padding:; line-height:0.4em;"><?php 
	echo $row['code'];?></td>
    <td style="text-align:center; color:#FFF ;height:1px; padding:; line-height:0.4em;"><a href="detail_all_range.php?channel_id=<?php echo $channel_id; ?>&from_kmpost=<?php echo $from_kmpost;?>&to_kmpost=<?php echo $to_kmpost?>&detail=<?php echo $row['code']; ?>&language=<?php echo $_SESSION['lang']; ?>" target="_blank" style="text-decoration:none"><?php echo $row['total']; ?></a></td>
</tr>
