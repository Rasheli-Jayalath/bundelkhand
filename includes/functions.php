...
function getProjectCommProgC($pid,$latest_month)
{

$latest_achieved=0;
$reportquery ="SELECT a.aid, a.baseline as milestoneBaseline, a.assig, c.cid,c.pid, c.cassig  FROM mis_tbl_4_milstones a 
inner join mis_tbl_3_subcomponents b on(a.s_id=b.s_id) inner join mis_tbl_2_components c on (c.cid=b.cid) where c.pid=".$pid." Group by c.pid,b.cid Order By c.pid";
$reportresult = mysql_query($reportquery);
$counter=mysql_num_rows($reportresult);
while ($reportdata = mysql_fetch_array($reportresult)) {
   ...
   if($latest_month!=0)
	  ...
	  if($reportdata['cid']<=2)
	  {
	  $latest_achieved=getComponentCommProgC($reportdata['cid'],$latest_month);
	  }
	  else
	  {
	  //$latest_month=explode('-',$latest_month);
	 
	  $latest_achieved=getComponentCommProg($reportdata['cid'],$latest_month);
	  
	  }
	  }
	if($latest_achieved!=0)
	 {
	 $comm_progress+=($latest_achieved)*($reportdata['cassig']/100);
	 $latest_achieved=0;
	 }
...
function getComponentCommProgC($cid,$latest_month)
{
...
while ($reportdata = mysql_fetch_array($reportresult)) {
  ...
  if($latest_month!=0)
  {
  $latest_achieved=getSubComponentCommProgC($reportdata['s_id'],$latest_month);
  }
if($latest_achieved!=0)
 {
 $comm_progress+=($latest_achieved)*($reportdata['Sassig']/100);
...
function getSubComponentCommProgC($s_id,$latest_month)
{
...
while ($reportdata = mysql_fetch_array($reportresult)) {
  ...
  if($latest_month!=0)
  {
  $ptodate=getMilestoneTotalAchieveCLatest($latest_month,$reportdata['aid']);
  }
...
function getProjectCommTargC($pid,$latest_month)
{
...
while ($reportdata = mysql_fetch_array($reportresult)) {
	...
	if($latest_month!=0)
	  ...
	  if($reportdata['cid']<=2)
	  {
	  $latest_target=getComponentCommTargC($reportdata['cid'],$latest_month);
	  }
	  else
	  {
	  
	  $latest_target=getComponentCommTarg($reportdata['cid'],$latest_month);
	  
	  }
	  }
	if($latest_target!=0)
	 {
	 $comm_progress+=($latest_target)*($reportdata['cassig']/100);
	 $latest_target=0;
	 }
...
function getComponentCommTargC($cid,$latest_month)
{
...
if($latest_month!=0)
  {
 $latest_target=getSubComponentCommTargC($reportdata['s_id'],$latest_month);
  }
...
function getSubComponentCommTargC($s_id,$latest_month)
{
...
while ($reportdata = mysql_fetch_array($reportresult)) {
  ...
  if($latest_month!=0)
  {
  $ptodate=getMilestoneTotalTargetsCLatest($latest_month,$reportdata['aid']);
  }
if($ptodate!=0&&$baseline!=0)
 {
	 $comm_progress+=($ptodate/$baseline)*($reportdata['assig']/100);
...
function getMilestoneTargetsCCC($month,$aid)
{
	//$sql="select  sum(prquantity) as milestone_targets from milestone_budget where mid =".$aid." and prmonth<='".$month."' order by mid";
	$sql="select  sum(prquantity) as milestone_targets from milestone_budget where mid =".$aid." AND prmonth<='".$month."' order by mid, prmonth";
	//$sql="SELECT sum(targets) as milestone_targets FROM  `test1` where aid=".$aid." And bid=".$bid;
...
function getMilestoneTargetsPC($month,$aid)
{
	//$sql="select  sum(prquantity) as milestone_targets from milestone_budget where mid =".$aid." and prmonth<='".$month."' order by mid";
	$sql="select  sum(prquantity) as milestone_targets from milestone_budget where mid =".$aid." AND prmonth='".$month."' order by mid, prmonth";
	//$sql="SELECT sum(targets) as milestone_targets FROM  `test1` where aid=".$aid." And bid=".$bid;
...
function getMilestoneTotalTargetsCLatest($latest_month,$aid)
{

 $sql="select  sum(prquantity) as total_milestone_targets from milestone_budget where mid =".$aid." AND prmonth<='".$latest_month."' order by mid, prmonth";
...
{
 $data=mysql_fetch_array($amountresult);
 $total_milestone_targets=$data["total_milestone_targets"];
}
else
{
...
function getMilestoneTotalAchieveCLatest($latest_month,$aid)
{

$sql="select  sum(prquantity) as total_milestone_achieve from milestone_progress where mid =".$aid." AND prmonth<='".$latest_month."' order by mid, prmonth";
	//$sql="SELECT sum(targets) as milestone_targets FROM  `test1` where aid=".$aid." And bid=".$bid;
 $amountresult = mysql_query($sql);
//echo $amountsize= mysql_num_rows($amountresult);
if($amountresult!=0)
{
 $data=mysql_fetch_array($amountresult);
 $total_milestone_targets=$data["total_milestone_achieve"];
}
else
{
$total_milestone_targets=0;
...
function getMilestoneTotalAchieveCLast($latest_month,$aid)
{
...
function getMilestoneTotalTargetsCLast($latest_month,$aid)
{
...
function getMilestoneTotalTargetsC($aid)
{
/*$sql="select  sum(prquantity) as total_milestone_targets from milestone_budget where mid =".$aid." AND prmonth<='".$latest_month."' order by mid, prmonth";*/
$sql="select sum(prquantity) as total_milestone_targets from milestone_budget where mid =".$aid." order by mid, prmonth";
	//$sql="SELECT sum(targets) as milestone_targets FROM  `test1` where aid=".$aid." And bid=".$bid;
 $amountresult = mysql_query($sql);
//echo $amountsize= mysql_num_rows($amountresult);
if($amountresult!=0)
...
function getMilestoneAchieveCCC($month,$aid)
{
...
function getMilestoneAchievePC($month,$aid)
{
